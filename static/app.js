...
async function send(text){
  ...
  try{
    ...
    if(data.demo) substate.textContent='Demo mode • add an OpenAI API key in Render for live AI';
...
async function checkHealth(){try{const r=await fetch('/api/health');const d=await r.json();if(d.configured){substate.textContent=`Ready • ${labels[d.model]||d.model}`;}else{substate.textContent='Demo mode • API key not configured';}}catch(e){substate.textContent='Server connection unavailable';}}
checkHealth();
renderHistory();
